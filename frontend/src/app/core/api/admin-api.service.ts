...
export class AdminApiService {
  ...
  specifications(categoryId: string): Observable<ReadonlyArray<AdminSpecificationDefinition>> {
    return this.http.get<ReadonlyArray<AdminSpecificationDefinition>>(
      `/api/v1/admin/categories/${categoryId}/specifications`,
    );
  }
  createSpecification(
    categoryId: string,
    body: SpecificationDefinitionWrite,
  ): Observable<AdminSpecificationDefinition> {
    return this.http.post<AdminSpecificationDefinition>(
      `/api/v1/admin/categories/${categoryId}/specifications`,
      body,
    );
  }
  updateSpecification(
    ...
    return this.http.put<AdminSpecificationDefinition>(
      `/api/v1/admin/categories/${categoryId}/specifications/${id}`,
      body,
      ifMatch(version),
    );
  }
  ...
  products(params: {
    ...
    page?: number;
    pageSize?: number;
  }): Observable<Page<AdminProductSummary>> {
    ...
    return this.http.get<Page<AdminProductSummary>>('/api/v1/admin/products', {
      params: httpParams,
    });
  }
  ...
  updateSpecificationValues(
    id: string,
    values: ReadonlyArray<SpecificationValueWrite>,
    version: number,
  ): Observable<void> {
    return this.http.put<void>(
      `/api/v1/admin/products/${id}/specifications`,
      { values },
      ifMatch(version),
    );
  }
  deleteProduct(id: string, version: number): Observable<void> {
    return this.http.delete<void>(`/api/v1/admin/products/${id}`, {
      ...ifMatch(version),
      headers: { ...ifMatch(version).headers, confirmation: 'permanently-delete' },
    });
  }
  ...
  reorderImages(productId: string, orderedImageIds: ReadonlyArray<string>): Observable<void> {
    return this.http.put<void>(`/api/v1/admin/products/${productId}/images/order`, {
      orderedImageIds,
    });
  }
  ...
  updateImageAltText(productId: string, imageId: string, altTextFa: string): Observable<void> {
    return this.http.put<void>(`/api/v1/admin/products/${productId}/images/${imageId}`, {
      altTextFa,
    });
  }
  ...
  updateManagedContent(
    slotKey: string,
    body: ManagedContentWrite,
    version: number,
  ): Observable<ManagedContentEntity> {
    return this.http.put<ManagedContentEntity>(
      `/api/v1/admin/site/content/${slotKey}`,
      body,
      ifMatch(version),
    );
  }
  ...
  }): Observable<AdministratorAccountDto> {
