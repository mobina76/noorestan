...
@Component({
  ...
  template: `
    <header class="page-head">
      ...
      <h1>محصولات نورستان</h1>
      <p>
        در میان محصولات مازی‌نور جست‌وجو کنید و برای انتخاب، قیمت و موجودی با نورستان تماس بگیرید.
      </p>
    </header>
    <section class="catalog">
      <aside>
        ...
        <fieldset>
          ...
          <button type="button" [class.active]="category() === null" (click)="setCategory(null)">
            همه محصولات
          </button>
          @for (item of categories(); track item.slug) {
            ...
            >
              {{ item.nameFa }} <small>({{ item.productCount }})</small>
            </button>
          }
        </fieldset>
        <div class="help">
          <span>محصول مناسب را پیدا نکردید؟</span><a href="/contact">از ما بپرسید ←</a>
        </div>
      </aside>
      <div>
          ...
          @for (product of products(); track product.id; let first = $first) {
            ...
            @if (!loading()) {
              <div class="empty">
                ...
                <button type="button" (click)="reset()">پاک‌کردن فیلترها</button>
              </div>
            }
          }
        </div>
